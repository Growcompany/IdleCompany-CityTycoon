...
1. **높이 비례 프레이밍** — 외부 5회 수렴 × 내부 15회 이진탐색으로 <code>ZoomValue</code>를 역산하고, <code>cos(pitch)</code>로 수직 압축을 보정했습니다. 첫 구현은 보정 방향이 반대였고 로그(줌값 0.079, 거리 7,258cm)로 발견해 수정했습니다. 당시엔 계산 중 바꾼 줌 상태를 복원하는 식으로 카메라 튐을 막았습니다(5에서 구조적으로 해소).
