...
## 6. 건물 포커스 프레이밍 — 요구가 세 번 바뀐 카메라

<div align="center">
  <img src="images/camera-framing-geometry.png" alt="카메라 프레이밍 기하 — 피치·팔길이·FOV·건물 높이와 탐색 루프" width="720">
  <br>
...
1. **높이 비례 프레이밍** — 외부 5회 수렴 × 내부 15회 이진탐색으로 <code>ZoomValue</code>를 역산하고, <code>cos(pitch)</code>로 수직 압축을 보정했습니다. 첫 구현은 보정 방향이 반대였고 로그로 발견해 수정했습니다. 당시엔 계산 중 바꾼 줌 상태를 복원하는 식으로 카메라 튐을 막았습니다(4에서 구조적으로 해소).
...
3. **시야 가림 고스트** — 포커스가 Yaw를 고려하지 않아 앞줄 건물이 타깃을 가렸습니다. 설계 단계에서 카메라를 뚫린 각도로 돌리는 안(밀집 지역엔 빈 각도가 없을 수 있음)과 건물 메시 자체를 반투명화하는 안(층 박스 수십 개를 한 ISM으로 그리는 구조라 반투명 정렬이 깨져 뒷면이 앞면을 뚫고 나오고, 창문 발광이 묽어짐. 모바일 예산 정책상 반투명 유리도 금지)을 기각하고, 라인트레이스 5발로 가림 건물을 찾아 본체를 숨기고 볼록 큐브 1개를 반투명 골조로 띄웁니다. 구현 중 건물 박스가 채널에 Block이라 광선당 건물이 1개만 잡혀 뒷줄 건물이 계속 가리던 문제는 조회자 응답을 Overlap으로 낮춰 광선이 끝까지 가게 해 해결했습니다. 판정(<code>UFocusOcclusionHandler</code>)과 표현(건물 액터)을 분리해 순수 수학층만 유닛 테스트로 덮습니다.
4. **수식 정정과 순수 모델 분리** — 세로 점유율 계산이 수평 <code>FieldOfView</code>의 tan을 그대로 쓰고(엔진 기본 제약 MaintainYFOV에서 수직 FOV는 <code>tan(FOV/2) / CameraComponent.AspectRatio</code>, 30°면 약 17°) 분모의 2를 빼먹어, 두 오류가 상쇄된 채 튜닝돼 있었습니다. 리그(팔길이·피치·FOV는 줌 key에 선형, 비선형은 <code>C_Zoom</code> 하나)를 순수 함수 <code>CameraFramingMath</code>로 분리하고, 잔차 <code>팔길이(z) − 필요 거리(z)</code> 하나에 대한 이분법(최대 24회, 오차 1cm)으로 줌값을 역산합니다. 탐색이 SpringArm을 건드리지 않아 복원 코드가 사라졌고, 순수 계산부는 자동화 테스트 6종으로 고정했습니다.
...
- [CameraFramingMath.cpp](../Source/CompanyGrowthRenewal/Private/Player/Components/CameraFramingMath.cpp) · [CameraFramingMathTests.cpp](../Source/CompanyGrowthRenewal/Private/Tests/CameraFramingMathTests.cpp)
...
- [포트폴리오 트러블슈팅 01 (Notion)](https://cookie-roquefort-35d.notion.site/2626907ae0ce8030b6edd2bdb5ae4931)
