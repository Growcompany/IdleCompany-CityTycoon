...
3. **시야 가림 고스트** — 포커스가 Yaw를 고려하지 않아 앞줄 건물이 타깃을 가렸습니다. 설계 단계에서 카메라를 뚫린 각도로 돌리는 안(밀집 지역엔 빈 각도가 없을 수 있음)과 건물 메시 자체를 반투명화하는 안(층별 ISM이라 정렬이 무너지고 창문 발광이 손실, Android 반투명 예산)을 기각하고, 라인트레이스 5발로 가림 건물을 찾아 본체를 숨기고 볼록 큐브 1개를 반투명 골조로 띄웁니다. 구현 중 건물 박스가 채널에 Block이라 광선당 건물이 1개만 잡혀 뒷줄 건물이 계속 가리던 문제는 조회자 응답을 Overlap으로 낮춰 광선이 끝까지 가게 해 해결했습니다. 판정(<code>UFocusOcclusionHandler</code>)과 표현(건물 액터)을 분리해 순수 수학층만 유닛 테스트로 덮습니다.
