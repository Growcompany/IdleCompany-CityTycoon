...
function generateReport(outDir, specDate, opts = {}) {
  ...
  if (!brick) {
    ...
    w(
      ...
      ``,
      `**적용 전 반드시 먼저 할 것 (순서):**`,
      `1. \`FBuildingData\` 의 \`FootprintWidthCells/DepthCells\` 를 추출기에 등재. **지금 이 값들은 \`BuildingDataTable.uasset\` 안에만 있고 CSV 가 없어**(\`DataImport/\` 에 부재) 위 표의 칸수 분포조차 설계 문서 인용이다 — 기계 검증이 불가능하다.`,
      `2. \`BuildableTable\` 도 CSV 가 없다. **DT → CSV 내보내기부터** 해야 곡선을 시드할 수 있다.`,
  ...
  w(
    ...
    `- **\`BuildingDataTable\` / \`BuildableTable\` 에 CSV 가 없다**(\`DataImport/\` 부재, uasset 전용). 그래서 §7 의 footprint 칸수 분포는 **설계 스펙 문서 인용**이고 기계 검증이 안 된다 — 건설비 곡선을 적용하려면 DT→CSV 내보내기가 선행돼야 한다.`,
